...
const collect_data = ["Prepare a dataset for all players", "Bereiten Sie einen Datensatz für alle Spieler vor", "Tüm oyuncular için bir veri kümesi hazırlayın", "Bereid een dataset voor alle spelers voor", "Przygotuj zestaw danych dla wszystkich graczy", "Preparar un conjunto de datos para todos los jugadores", "Förbered ett dataset för alla spelare", "Préparer un ensemble de données pour tous les joueurs", "Подготовьте набор данных для всех игроков", "为所有玩家准备一个数据集", "全選手のデータセットを用意する", "Preparar um conjunto de dados para todos os jogadores", ];
...
const onderdeel_test = ["Experience the problem of getting perfection", "Erleben Sie das Problem, Perfektion zu erreichen", "Mükemmelliğe ulaşma sorununu yaşayın", "Ervaar het probleem van perfectie", "Doświadczenie problemu z osiągnięciem doskonałości", "Experimentar el problema de conseguir la perfección", "Upplev problemet med att få perfektion", "Expérimenter le problème de l'obtention de la perfection", "Испытайте проблему достижения совершенства", "体验追求完美的问题", "完璧を求める問題を 体験する", "Experimentar o problema de obter a perfeição", ];
...
const tel_nauwkeurig = ["Can you count?", "Können Sie zählen?", "Sayabilir misin?", "Kun je tellen?", "Umiesz liczyć?", "¿Sabes contar?", "Kan du räkna?", "Savez-vous compter ?", "Вы умеете считать?", "你会数数吗？", "数えられるか？", "Consegues contar?", ];
const ben_je_zeker=["Are you sure?", "Sind Sie sicher?", "Emin misin?", "Weet je het zeker?", "Jesteś pewien?", "¿Seguro?", "Är du säker?", "Êtes-vous sûr ?", "Вы уверены?", "你确定吗？", "本当か？", "Tens a certeza?", ];
...
function zettaal(taalIndex) {
    ...
    $(".ben_je_zeker").text(ben_je_zeker[taalIndex]);
